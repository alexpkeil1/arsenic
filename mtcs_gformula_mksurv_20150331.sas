...
%INCLUDE "Z:/EpiProjects/MT_copper_smelters/code/mtcs_gformula_20140908.sas";
...
PROC PHREG DATA = ___sur;
 ...
 BASELINE out=gformula.surv_lw_&outvar._&ds CUMHAZ=cumhaz_lw  / METHOD=FH;
RUN;
PROC PHREG DATA = ___sur;
 WHERE returnwork=1 OR (activework=0 AND leavework=0);
 MODEL (&outvar.in_alt &outvar.out)*returnwork(0) = / TIES=EFRON;
 BASELINE out=gformula.surv_rw_&outvar._&ds CUMHAZ=cumhaz_rw / METHOD=FH;
RUN;

DATA gformula.surv_rw_&outvar._&ds; SET gformula.surv_rw_&outvar._&ds;&outvar.out = &outvar.out + &mintime;
DATA gformula.surv_lw_&outvar._&ds; SET gformula.surv_lw_&outvar._&ds;&outvar.out = &outvar.out + &mintime;
...
DATA gformula.cidata_&outvar._&ds (KEEP = &outvar.out surv_ac km_: ci_: altci: auc_:);
